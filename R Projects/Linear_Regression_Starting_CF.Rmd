...
pressure_plot <- ggplot(pressure, aes(temperature, pressure)) + geom_point()
pressure_plot
```
...
coefficient <- cor.test(pressure$temperature, pressure$pressure)
coefficient$estimate
#0.7577923 < therefore a strong correlation between variables. 

coefficient_cars <- cor.test(cars$speed, cars$dist)
coefficient_cars$estimate
#0.8068949 < therefore a strong correlation between speed and distance in the cars dataset.

coefficient_states <- cor.test(state_df$Illiteracy, state_df$Income)
coefficient_states$estimate
# -0.4370752 < mild correlation but not overly strong between Income and Illiteracy in the states dataset. 
```
...
cars_boxplot <- ggplot(cars, aes(speed)) + geom_boxplot() + labs(x="Speed")
cars_boxplot

state_df <- as.data.frame(state.x77)
state_boxplot <- ggplot(state_df, aes(Illiteracy)) + geom_boxplot()
state_boxplot
```
...
sigma(car_model) #[1] 15.35049
sigma(state_model) #[1] 627.6051
```
...
summary(car_model)$r.squared #[1] 0.7110617
summary(state_model)$r.squared #[1] 0.1964859
```
...
#CF Comments: based on the model summary for the Cars dataset, the coefficients of speed against distance is 4.275, meaning that for every 1 distance, the speed goes up by 4.275?
...
#CF Comments: based on the model summary for the States dataset, the coefficients of Illiteracy against Income is -488.3, meaning that for every 1 income, the speed goes down by -488.3?
```
...
cars_plot <- cars_train %>% ggplot(aes(speed, dist)) + geom_point(aes(size=residuals)) + geom_point(aes(y=estimate), color = "blue") + geom_segment(aes(xend = speed, yend=estimate), color="gray")
cars_plot

state_plot <- state_train %>% ggplot(aes(Illiteracy, Income)) + geom_point(aes(size=residuals)) + geom_point(aes(y=estimate), color = "blue") + geom_segment(aes(xend = Illiteracy, yend=estimate), color="gray")
state_plot
```
...
loess_car <- ggplot(cars_train, aes(speed, dist)) + geom_point() + geom_smooth(method="lm") + geom_smooth(se=FALSE, color="red")
loess_car

loess_state <- ggplot(state_train, aes(Illiteracy, Income)) + geom_point() + geom_smooth(method="lm") + geom_smooth(se=FALSE, color="red")
loess_state
```
